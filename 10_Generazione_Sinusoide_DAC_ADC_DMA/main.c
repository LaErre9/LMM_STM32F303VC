...
int main()
{
  ...
  //ABILITAZIONE GPIOA - ADC12 - DAC1- TIM2 - DMA1 - DMA2
  ...
  //-------------------------CONFIGURAZIONE DMA1
  DMA1_Channel1->CPAR = (unsigned int) &ADC1->DR; //address periferica
  DMA1_Channel1->CMAR = (unsigned int) vett_ADC;        //address memroia
  DMA1_Channel1->CNDTR = N;                             //numero trasferimenti
  DMA1_Channel1->CCR |= DMA_CCR_MSIZE_0;                //msize da 16bit
  DMA1_Channel1->CCR |= DMA_CCR_PSIZE_0;                //psize da 16bit
  DMA1_Channel1->CCR |= DMA_CCR_MINC;                   //incremento indirizzo memoria
  DMA1_Channel1->CCR &= ~DMA_CCR_DIR;                   //DIR = 0 e periferica->memoria
  DMA1_Channel1->CCR |= DMA_CCR_EN;                     //abilitaione canale 1
  
  //------------------------CONFIGURAZONE DMA2
  DMA2_Channel3->CPAR = (unsigned int) &(DAC->DHR12R1);  //address periferica
  DMA2_Channel3->CMAR = (unsigned int) vett_DAC;
  DMA2_Channel3->CNDTR = N;
  DMA2_Channel3->CCR |= DMA_CCR_MSIZE_0;        //msize a 16bit
  DMA2_Channel3->CCR |= DMA_CCR_PSIZE_0;        //psize a 16bit
  DMA2_Channel3->CCR |= DMA_CCR_MINC;           //incremento indirizzo memoria
  DMA2_Channel3->CCR |= DMA_CCR_CIRC;           //modalità circolare
  DMA2_Channel3->CCR |= DMA_CCR_DIR;            //DIR a 1 è memoria -> periferica
  ...
  DAC->CR |= DAC_CR_DMAEN1;             //modalità DMA del DAC
  ...
  ADC1->CFGR &= ~ADC_CFGR_CONT;          //cont = 1, conversione singola
      ...
      //configurazione ADC DMA
  ADC1->CFGR |= ADC_CFGR_DMAEN;         //modalità DMA
      //configurazione TRIGGER 
  ADC1->CFGR |= ADC_CFGR_EXTEN_0;       //trigger abilitati a fronte di salita
  ADC1->CFGR |= (11<<6);                //EXTSEL=1011: trigger del TIM2
  
  ADC1->SQR1 = (3<<6);                   //SQR1=00011: canale 3 (PA2)
  ...
  *  Deltat    = 100ms = 0.1s(tempo che vogliamo)
  *  Deltat    = N * tupdate (N = numero campioni) 
  *  tupdate   = Deltat/Ncampioni = (0.1)/(100) = (10^-3) (tempo di refresh)
  *  N         = tupdate/Tck = 72000 (Numero dei conteggi)
  *  dunque TIM2_ARR = 72000. 
  */
  
  TIM2->ARR = 72000;            //0,1 s
  ...
  ADC1->CR |= ADC_CR_ADSTART;   //Avvui conversione
